...
export default function StaffLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        // Store session token
        localStorage.setItem("bc_staff_session", data.sessionToken);
        navigate("/bc-sp=panel");
      } else {
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center space-x-3 mb-4">
            <div className="relative">
              <div className="w-16 h-16 bg-gradient-to-br from-neon-blue to-blood-500 rounded-lg flex items-center justify-center">
                <div className="w-10 h-10 bg-white rounded-sm transform rotate-45 opacity-90" />
              </div>
              <div className="absolute inset-0 bg-gradient-to-br from-neon-blue to-blood-500 rounded-lg blur-sm opacity-50" />
            </div>
          </div>
          <h1 className="text-3xl font-bold mb-2">
            <span className="blood-text gothic-font">Blood Cloud</span>
            <span className="text-neon-blue ml-2">Staff</span>
          </h1>
          <p className="text-foreground/70">Enter your staff credentials to access the panel</p>
        </div>
        ...
        <div className="card-blood p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3 px-6 bg-gradient-to-r from-neon-blue to-blood-500 hover:from-neon-blue/80 hover:to-blood-400 text-white font-semibold rounded-lg transition-all duration-300 hover:shadow-lg hover:shadow-neon-blue/30 hover:scale-105 active:scale-95 border border-neon-blue/30 group relative overflow-hidden"
            >
              <span className="relative z-10 flex items-center justify-center space-x-2">
                <span>{isLoading ? "Logging in..." : "Access Staff Panel"}</span>
                <ArrowRight className={`w-5 h-5 transition-transform ${isLoading ? "animate-pulse" : "group-hover:translate-x-1"}`} />
              </span>
              {isLoading && (
                <div className="absolute inset-0 bg-gradient-to-r from-neon-blue to-blood-500 animate-pulse" />
              )}
            </button>
          </form>
